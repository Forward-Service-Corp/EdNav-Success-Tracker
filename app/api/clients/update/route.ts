    ...
    if (clientId) {
      if (trackable) {
        ...
        await collection.updateOne(
          ...
          {
            $set: {
              [`trackable.items.${trackable}.completed`]: completed,
              [`trackable.items.${trackable}.createdAt`]: new Date().toISOString(),
              [`trackable.items.${trackable}.name`]: name,
              [`trackable.items.${trackable}.itemIndex`]: parseInt(trackable)
            }
          }
        );
      }
      ...
      const newestActivity = await activities.findOne({ _id: activity.insertedId }, { sort: { createdAt: -1 } });
      const client = await collection.findOne({ _id: safeObjectId(_id ?? clientId) });

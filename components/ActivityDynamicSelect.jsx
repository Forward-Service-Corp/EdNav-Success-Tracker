...
const ActivityDynamicSelect = ({ setOpen, questions = {}, onSuccess }) => {
  ...
  useEffect(() => {
    if (pendingClientUpdate) {
      ...
      setTimeout(() => {
        ...
        if (clientToUpdate.trackable && Array.isArray(clientToUpdate.items)) {
          const trackableItems = clientToUpdate.items || [];
  ...
  const activateClientSection = (sectionName) => {
    ...
    const updatedClient = {
      ...
      [normalizedSectionName]: {
        ...
        referralDate:
          selectedClient[normalizedSectionName]?.referralDate ||
          today.toISOString(),
    ...
    if (selectedClient?.items && Array.isArray(selectedClient.items)) {
      ...
      setTimeout(() => {
        ...
        if (section) {
          ...
          if (overlay) {
            ...
            console.error(
              `Could not find overlay in section ${normalizedSectionName}`,
            );
          }
          ...
          if (card) {
            ...
            console.error(
              `Could not find card in section ${normalizedSectionName}`,
            );
          }
        } else {
          console.error(
            `Could not find section with id: ${normalizedSectionName}`,
          );
  ...
  const updateClientTrackableItems = (
    ...
    try {
      ...
      if (updatedClient.trackable) {
        ...
        updatedClient.items = Array.isArray(updatedClient.items);
        updatedClient.trackable = {
          ...updatedClient.trackable,
  ...
  const saveSelectionToMongoDB = async (newPath, multi) => {
    try {
      ...
      const data = {
        ...
        selections:
          multi && Array.isArray(multiSelectValues)
            ? [...multiSelectValues]
            : null,
      };
      ...
      if (multi) {
        ...
        if (Array.isArray(items) && items.length > 0) {
          ...
          let updatedTrackable = {
            ...trackable,
            program:
              trackable.program ||
              (Array.isArray(selectedPath) && selectedPath.length > 1
                ? selectedPath[1]
                : "GED"),
        ...
        if (Array.isArray(newPath)) {
          try {
            hasGedHsed = newPath.some(
              (p) =>
                p === "GED" ||
                p === "HSED" ||
                (typeof p === "string" &&
                  (p.includes("GED") || p.includes("HSED"))),
            );
          } catch (e) {
      ...
      if (typeof window !== "undefined") {
        try {
          ...
          // Fall back to the original approach as a last resort
            ...
            console.error(
              "ACTIVITY DEBUG: No activity display method available!",
            );
          }
      ...
      try {
        ...
        if (data.clientId) {
          if (typeof data.clientId === "string") {
            ...
            if (data.clientId._id) {
              clientIdStr =
                typeof data.clientId._id === "string"
                  ? data.clientId._id
                  : data.clientId._id.toString();
        ...
        if (typeof window !== "undefined") {
          ...
          if (snExists && typeof snExists === "function") {
            ...
            window.alert(
              "Error saving activity: " + (error.message || "Unknown error"),
            );
          }
        }
  ...
  const handleAdvance = async () => {
    ...
    try {
      ...
      if (selectedValue === "GED" || selectedValue === "HSED") {
        try {
          const nextLevel = Array.isArray(newObject)
            ? newObject
          ...
          if (selectedClient) {
            ...
            if (typeof window !== "undefined") {
              try {
                ...
                console.error(
                  "Failed to save trackable data to localStorage:",
                  e,
                );
              }
            }
      ...
      if (newObject && typeof newObject === "object") {
        ...
        if (Array.isArray(newObject)) {
          ...
          try {
            ...
            if (
              (selectedPath[0] === "adult" || selectedPath[0] === "youth") &&
              ...
              if (
                ...
                try {
                  const savedTrackable = localStorage.getItem(
        ...
        try {
          setMultiSelectOptions(
            (newObject && newObject.completed) || newObject,
          );
          setCurrentOptions([]);
        } catch (e) {
        ...
        try {
          ...
          if (
            result &&
            selectedActivity &&
            Array.isArray(selectedActivity.activities)
          ) {
            setSelectedActivity({
              ...selectedActivity,
  ...
  const handleMultiSelectChange = (option, index) => {
    ...
    const isAlreadySavedInDatabase =
      Array.isArray(items) &&
      index < items.length &&
      items[index]?.savedInDatabase === true;
          ...
          if (Array.isArray(items) && index < items.length) {
            ...
            if (updatedItems[index]) {
              ...
              if (!updatedItems[index].savedInDatabase) {
                ...
                const updatedTrackable = { ...trackable };
                ...
                if (selectedClient) {
                  ...
                  if (typeof window !== "undefined") {
                    try {
                      ...
                      console.error(
                        "Failed to save trackable to localStorage:",
                        e,
                      );
                    }
                  }
          ...
          if (Array.isArray(items) && index < items.length) {
            ...
            if (updatedItems[index]) {
              ...
              const currentSavedStatus =
                updatedItems[index].savedInDatabase || false;
  ...
  const handleMultiSelectAdvance = async () => {
    ...
    try {
      ...
      try {
        ...
        if (result && result.trackable && Array.isArray(result.items)) {
        ...
        let errorMessage =
          "Activity was saved locally but there was an issue with the server.";
        if (saveError && saveError.message) {
          if (saveError.message.includes("Missing clientId")) {
            errorMessage =
              "Activity was saved locally. Client information issue on server.";
          } else if (saveError.message.includes("Client not found")) {
            errorMessage =
              "Activity was saved locally. Client not found on server.";
          } else if (saveError.message.includes("500")) {
            errorMessage =
              "Activity was saved locally, but there was a server issue.";
          }
        }
  ...
  const getCompletedTrackableItems = () => {
    ...
    if (selectedClient?.items && Array.isArray(selectedClient.items)) {
      completedItems = selectedClient.items
    ...
    if (
      ...
      try {
        ...
        console.error(
          "Error checking localStorage for completed trackable items:",
          e,
        );
      }
    }
  ...
  return (
    <div className="relative z-50">
      ...
      {finalSelection && finalSelection.startsWith("Error:") && (
        <div className="mt-4 text-red-600">{finalSelection}</div>
      )}
          ...
          <div className="grid grid-cols-1 gap-2">
            {Array.isArray(multiSelectOptions) &&
              multiSelectOptions.map((option, index) => {
                ...
                if (Array.isArray(items) && index < items.length) {
                  isSavedInDatabase = items[index]?.savedInDatabase === true;
                }
                ...
                if (
                  ...
                  Array.isArray(selectedClient.items)
                ) {
                  isSavedInDatabase = selectedClient.items.some(
                    (item) =>
                ...
                return (
                  <label
                    ...
                    title={
                      isSavedInDatabase
                        ? "This item is saved in the database and cannot be changed"
                        : isCompleted
                          ? "This item is selected but not yet saved to the database"
                  ...
                  >
                    <input
                      ...
                      value={option}
                      name={
                        selectedPath[selectedPath.length - 1] || "firstOption"
                      }
                      checked={
                        (Array.isArray(multiSelectValues) &&
                    ...
                    {statusLabel && (
                      <span
                        className={`ml-2 text-xs ${isSavedInDatabase ? "text-green-500" : "text-yellow-500"}`}
                      >
                        {statusLabel}
                      </span>
                    )}

...
function ProgressButton({ item, index, isDisabled }) {
  ...
  const saveItem = async () => {
    ...
    try {
      statement = generateSentence(
        ...
        `${selectedClient.first_name || ""} ${selectedClient.last_name || ""}`.trim() ||
          "Client",
        item?.name || null,
        Array.isArray([selectedClient?.trackable?.program])
          ? [selectedClient?.trackable?.program]
          : [],
      );
    } catch (error) {
    ...
    try {
      const res = await fetch(
        `/api/clients/update?clientId=${selectedClient?._id}&trackable=${index}`,
        {
          method: "POST",
          body: JSON.stringify({
            completed: true,
  ...
  return (
    ...
    >
      {item?.completed === true ? (
        <span
          className={`border-success text-success flex cursor-not-allowed items-center justify-center rounded-full border pr-2`}
        >
          <span className={`mr-1`}>

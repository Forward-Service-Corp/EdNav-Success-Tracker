...
function ClientProfileUnlockableSection({ isNarrow, section }) {
  ...
  return (
    <div id="orientation" className={`relative`}>
      <div
        className={`absolute top-0 right-0 bottom-0 left-0 flex items-center justify-center ${selectedClient[section]?.referralDate ? 'invisible' : 'visible'}`}
      >
        <div
      ...
      <div
        className={`card card-sm bg-base-200 border-base-content/10 relative mx-3 md:mr-6 rounded-lg border-1 p-3 md:p-6 shadow ${selectedClient[section]?.referralDate ? '' : 'opacity-50 blur-[2px]'}`}
      >

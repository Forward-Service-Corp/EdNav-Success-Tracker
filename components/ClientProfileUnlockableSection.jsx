...
function ClientProfileUnlockableSection({ isNarrow, section }) {
  ...
  return (
    <div id={section} className={`relative`}>
      ...
      <div
        className={`bg-base-200 border-base-content/10 relative   rounded-lg border-1 p-3 md:p-6 shadow ${selectedClient[section] ? '' : 'opacity-50 blur-[2px]'}`}>
        <div className={`${isNarrow ? 'text-lg' : 'text-xl'} uppercase font-light tracking-wider`}>{section}</div>

...
export default function ClientTable({ menuOpen, setMenuOpen }) {
  ...
  }, [filteredClients, viewMode, pinnedIds, groupByClientStatus]);
  ...
  return (
    <div className={`relative h-full`}>
      <div className="bg-base-100 sticky top-0 z-10 h-[80px] w-full shadow-lg">
        <SearchField
          menuOpen={menuOpen}
      ...
      <div className={`relative h-full w-full`}>
        <div
          className={`no-scrollbar max-h-[calc(100vh-80px)] w-full overflow-y-auto`}
        >
          <table
            className={`w-full ${!filterOpen ? "" : "mt-[45px]"} table-auto`}
          >
            <thead className="w-full">
              <tr className="w-full text-left text-xs">
                <th className={`pl-6`}>Name</th>
                <th className={`${menuOpen ? "hidden" : ""}`}>Last Activity</th>
                <th className={`${menuOpen ? "hidden" : ""}`}>Group</th>
                <th className={`${menuOpen ? "hidden" : ""}`}>FEP</th>
                <th className={`${menuOpen ? "hidden" : ""}`}>Region</th>
                <th className={``}>Pin</th>
                <th
                  className={`col-span-1 cursor-pointer items-center py-2 text-sm`}
                >
                  Status
                </th>
              </tr>
              ...
              {viewMode === "grouped" ? (
                Object.entries(clientsToShow).map(([status, clients], idx) => (
                  <React.Fragment key={status}>
                    ...
                    >
                      <td
                        className={`cursor-pointer items-center py-2 text-sm`}
                      >
                        <span className={`ml-6 font-bold`}>{status}</span>
                      </td>

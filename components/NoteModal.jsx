...
export default function NoteModal({ open, setOpen, onSuccess }) {
  ...
  return (
    <Dialog open={open === 'note'} onClose={() => setOpen('')} className="">
      <DialogBackdrop
        transition
      ...
      <div className="fixed w-screen overflow-y-auto">
        <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
          <DialogPanel
            transition
            className="relative transform overflow-hidden rounded bg-base-100 p-12 text-left shadow transition-all data-[closed]:translate-y-4 data-[closed]:opacity-0 data-[enter]:duration-300 data-[leave]:duration-200 data-[enter]:ease-out data-[leave]:ease-in sm:my-8 sm: sm:max-w-lg sm:p-6 data-[closed]:sm:translate-y-0 data-[closed]:sm:scale-95"
          >
            <div className="px-12 py-8">
              <div>
                ...
                <div className="flex flex-col mt-4 gap-4">
                  <textarea className="textarea textarea-bordered " placeholder="Note" value={note.noteContent}
